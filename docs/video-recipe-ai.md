...
- 마지막에 **자체 CRF NER**로 재료 정규화("대파 반 단"→표준 품목) — **냉장고 재고 매칭 + 쿠팡 장바구니 가격 비교의 연결 키** (design.md §4.3)
...
flowchart TD
    ...
    K1 --> H
    H --> I[ES nori 유사도 검색<br/>→ 쿠팡 상품 매핑]
...
| **텍스트 우선·Gemini 폴백 (폴백 반전안)** | 설명란/자막 텍스트로 먼저 추출 시도(무료) → 부실할 때만 Gemini 영상 분석. 성립 시 비용 추가 하락, 단 자막 없는 영상·쇼츠에서 품질 리스크 | 🟡 PoC에서 품질 비교 후 결정 |
