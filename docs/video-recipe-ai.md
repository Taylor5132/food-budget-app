...
> **대상 독자:** PoC 검증 담당자
> **상태:** 파이프라인 구조·모델 선정 **확정** / 캐싱 정책 **미결** (아래 §6)
> **결정일:** 2026-07-09 · 상세 배경: `design.md` §3 / §8 결정 로그
...
flowchart TD
    ...
    B -->|통과| C[1차 추출: Gemini 3.1 Flash-Lite<br/>영상 → JSON 초안 + 타임스탬프<br/>~$0.047]
    ...
    D2 -->|통과| G
    G --> H[CRF NER 재료 정규화<br/>자체 AI · CPU]
    H --> I[레시피 저장<br/>스텝 텍스트 + JSON + 타임스탬프]
    I --> J[냉장고 diff · 장바구니 연동]
```
...
| 항목 | 내용 | 상태 |
|---|---|---|
| **캐싱 정책** | design.md 가드레일 "유저 건별·개인용·**비저장**"과 결과물 캐싱 충돌. 후보: A 전면 캐싱(파생 요약 저장+출처 표기) / B 유저 스코프 캐싱(절감 미미) / C TTL 30일 캐싱(균형) | 🟡 팀 회의 대기 |
...
**산출물:** 위 결과를 본 문서에 추가 커밋 (§8 검증 결과 섹션 신설) + design.md 결정 로그 갱신
